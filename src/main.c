...
void setup(void) {
    ...
    init_camera(vec3_new(0, 0, 0), vec3_new(0, 0, 1), vec3_new(0, 0, 0), 0.0, 0.0);
        ...
        switch (event.type) {
            ...
            case SDL_KEYDOWN:
                ...
                if (event.key.keysym.sym == SDLK_UP) {
                    camera_increase_pos_y(3.0 * delta_time);
                    break;
                }
                if (event.key.keysym.sym == SDLK_DOWN) {
                    camera_decrease_pos_y(3.0 * delta_time);
                    break;
                }
                if (event.key.keysym.sym == SDLK_a) {
                    camera_decrease_yaw(1.0 * delta_time);
                    break;
                }
                if (event.key.keysym.sym == SDLK_d) {
                    camera_increase_yaw(1.0 * delta_time);
                    break;
                }
                if (event.key.keysym.sym == SDLK_q) {
                    camera_increase_pitch(1.0 * delta_time);
                    break;
                }
                if (event.key.keysym.sym == SDLK_e) {
                    camera_decrease_pitch(1.0 * delta_time);
                    break;
                }
                if (event.key.keysym.sym == SDLK_w) {
                    camera_move_forward(5.0 * delta_time);
                    break;
                }
                if (event.key.keysym.sym == SDLK_s) {
                    camera_move_backward(5.0 * delta_time);
                    break;
                }
                break;
        }
...
void update(void) {
    ...
    for (int mesh_index = 0; mesh_index < get_num_meshes(); mesh_index++) {
        ...
        vec3_t target = { 0, 0, 1 };
        camera_rotate(target);

        // Offset the camera position in the direction where the camera is pointing at
        target = vec3_add(camera_get_position(), camera_get_direction());
